...
def adjustData(img,mask,flag_multi_class,num_class):
    if(flag_multi_class):
        img = img / 255
        #这里为什么取0 ，因为前面数据生成器里面用了灰度色彩模式，本来是二维数据，但是要构造成3维数据，
        #所以通道只有一个通道，保存在形状为（num，weight，height，1）的图里面，就是使用0的原因
        #取一个mask 的局部变量切片，不影响本来的mask
        mask = mask[:,:,:,0] if(len(mask.shape) == 4) else mask[:,:,0] #防止只有一张图的情况下少了容量维度
        #mask前面增加一个——类别维度
...
def trainGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,image_color_mode = "grayscale",
    ...
    image_generator = image_datagen.flow_from_directory(
        ...
        save_to_dir = save_to_dir,                  #若需要save生成器的训练结果
        save_prefix  = image_save_prefix,           #保存前缀 image
        seed = seed)
...
#其实这步我还是有点没懂，看了注释后讨论讨论
def labelVisualize(num_class,color_dict,img):
    img = img[:,:,0] if len(img.shape) == 3 else img
    img_out = np.zeros(img.shape + (3,))
    for i in range(num_class):
        img_out[img == i,:] = color_dict[i]  #还是这个语法看不懂
    return img_out / 255
